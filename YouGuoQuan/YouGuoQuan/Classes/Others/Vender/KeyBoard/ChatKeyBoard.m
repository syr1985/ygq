...
@property (nonatomic, strong) NSMutableString *textViewString;
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        ...
        [self addObserver:self
               forKeyPath:@"self.chatToolBar.frame"
...
{
    // 键盘已经弹起时，表情按钮被选择
    CGFloat supviewH = getSupviewH(self.keyboardInitialFrame);
    CGFloat selfViewH = CGRectGetHeight(self.frame);
    if (self.chatToolBar.faceSelected && (supviewH - CGRectGetMidY(self.frame)) < selfViewH)
    {
        ...
        [UIView animateWithDuration:0.25 delay:0.1 options:UIViewAnimationOptionCurveLinear animations:^{
            self.frame = CGRectMake(0, supviewH - selfViewH, kScreenWidth, selfViewH);
...
- (void)chatToolBar:(ChatToolBar *)toolBar voiceBtnPressed:(BOOL)select keyBoardState:(BOOL)change
{
    if (select && change == NO) {
        ...
        [UIView animateWithDuration:0.25 animations:^{
            ...
            CGFloat y = getSupviewH(self.keyboardInitialFrame) - self.chatToolBar.frame.size.height;
...
- (void)facePanelFacePicked:(FacePanel *)facePanel faceSize:(NSInteger)faceSize faceName:(NSString *)faceName delete:(BOOL)isDelete
{
    ...
    if (isDelete) {
        if (text.length > 1) {
            [self.chatToolBar deleteBackward];
        } else {
...
- (void)setKeyBoardStyle:(KeyBoardStyle)keyBoardStyle
{
    ...
    if (keyBoardStyle == KeyBoardStyleComment) {
        self.frame = CGRectMake(0, self.frame.origin.y, self.frame.size.width, self.frame.size.height);//+kChatToolBarHeight
    }
}
