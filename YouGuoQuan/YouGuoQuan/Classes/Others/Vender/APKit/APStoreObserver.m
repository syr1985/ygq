    ...
    for (SKPaymentTransaction *transaction in transactions) {
        switch (transaction.transactionState) {
          ...
          case SKPaymentTransactionStateFailed: {
                ...
                //[NSString stringWithFormat:@"Purchase of %@ failed.",transaction.payment.productIdentifier];
...
- (void)completeTransaction: (SKPaymentTransaction *)transaction forStatus: (APPurchaseStatus)status {
    self.status = status;
    // In Apple's Demo, they said: `Do not notify the user when purchase cancelled.`
    if (transaction.error.code == SKErrorPaymentCancelled ||
        [transaction.error.localizedDescription isEqualToString:@"Canceled by user."] ||
        [transaction.error.localizedDescription containsString:@"Cancel"]) {
        self.errorMessage = @"用户已取消";
