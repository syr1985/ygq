...
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *rechargeButtonWidthConstraint;
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *rechargeButtonTrailingConstraint;
@end
...
- (void)viewDidLoad {
    ...
    self.rechargeButtonWidthConstraint.constant = [LoginData sharedLoginData].ope ? 0 : 84;
    self.rechargeButtonTrailingConstraint.constant = [LoginData sharedLoginData].ope ? 0 : 12;
}
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [NetworkTool getMyWalletInfoSuccess:^(id result){
        ...
        [SVProgressHUD showErrorWithStatus:@"获取钱包数据失败"];
    }];
}
