...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
    ...
    if ([string isEqualToString:@""]) {
        if (textField.text.length > 2) {
            ...
            [self.certifiedInfoArray removeAllObjects];
            [textArray enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                NSString *subText = (NSString *)obj;
                if (![subText containsString:@" "]) {
                    [self.certifiedInfoArray addObject:subText];
                }
            }];
        } else if (textField.text.length == 2) {
            text = [textField.text substringToIndex:1];
            [self.certifiedInfoArray removeAllObjects];
            [self.certifiedInfoArray addObject:text];
        } else {
            [self.certifiedInfoArray removeAllObjects];
        }
        //NSLog(@"%@",self.certifiedInfoArray);
    } else {
