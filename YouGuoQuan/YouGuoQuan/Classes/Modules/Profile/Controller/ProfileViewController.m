...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(officiallyCertifiedSuccess:)
                                                 name:kNotification_OfficiallyCertifiedSuccess
                                               object:nil];
...
- (void)officiallyCertifiedSuccess:(NSNotification *)noti {
    if (_userBaseInfoModel) {
        NSDictionary *userInfo = noti.userInfo;
        _userBaseInfoModel.audit = [userInfo[@"type"] intValue];
    }
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context {
    ...
    if (![oldValue isEqual:[NSNull null]] && ![oldValue isEqualToString:newValue]) {
        [self.tableView reloadData];
        //[self updateTableViewWithIndexPaths:@[[NSIndexPath indexPathForRow:0 inSection:0]]];
    }
}
...
- (void)getUserTrendsList {
    ...
    [NetworkTool getOtherTrendsWithPageNo:@(_pageNo) pageSize:@(_pageSize) userID:[LoginData sharedLoginData].userId success:^(id result) {
        ...
        if (weakself.pageNo == 1) {
            ...
            //__block NSMutableIndexSet *reloadSectionSet = [NSMutableIndexSet indexSet];
            [sortSetArray enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                ...
                //[reloadSectionSet addIndex:resultArray.count];
            }];
            ...
            __block NSMutableIndexSet *reloadSectionSet = [NSMutableIndexSet indexSet];
            __block NSMutableIndexSet *insertSectionSet = [NSMutableIndexSet indexSet];
            [sortSetArray enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                if ([titlesArray containsObject:obj]) {
                    ...
                    if (index >= 0 && index < resultArray.count) {
                        [reloadSectionSet addIndex:index + 1];
                    ...
                    [resultArray addObject:muArray];
                    [insertSectionSet addIndex:resultArray.count];
                }
            }];
            ...
            if (reloadSectionSet.count) {
                [weakself updateTrendsInfoWithIndexPath:reloadSectionSet insert:NO];
            }
            if (insertSectionSet.count) {
                [weakself updateTrendsInfoWithIndexPath:insertSectionSet insert:YES];
            }
        }
    } failure:^{
        if (weakself.pageNo == 1) {
...
- (void)updateTableViewWithIndexPaths:(NSArray *)indexPaths {
    __weak typeof(self) weakself = self;
...
- (void)updateTrendsInfoWithIndexPath:(NSIndexSet *)indexSet insert:(BOOL)insert  {
    __weak typeof(self) weakself = self;
    dispatch_async(dispatch_get_main_queue(), ^{
        //[weakself.tableView beginUpdates];
        if (insert) {
            [weakself.tableView insertSections:indexSet withRowAnimation:UITableViewRowAnimationNone];
        } else {
        ...
        [NetworkTool uploadImage:imageData progress:^(NSString *key, float percent) {
            ...
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.25 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                [NetworkTool modifyPersonInfoWithType:@"coverImgUrl" value:url success:^{
                    ...
                    // 刷新
                    strongself.userBaseInfoModel.coverImgUrl = url;
...
//    }
//    [UploadImageTool pornImageWithImages:dictArray orignal:YES success:^(NSArray *urlArray) {
//        [SVProgressHUD dismiss];
//        __strong typeof(self) strongself = weakself;
//        if (isCover) {
//            // 调接口
...
//            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.25 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
//                [SVProgressHUD showErrorWithStatus:@"图片上传失败，请重新上传"];
//            });
//        } else {
//            if (sexImageArray) {
//                if (isCover) {
//                    [AlertViewTool showAlertViewWithTitle:@"提示" Message:@"图片违反了相关规定，请重新上传" buttonTitle:@"确定" sureBlock:^{
//                        
//                    }];
//                } else {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ...
        cell.publishBlock = ^(NSString *vcId){
            [PublishTool presentViewControllerWithIdentifier:vcId presenting:weakself];
        };
