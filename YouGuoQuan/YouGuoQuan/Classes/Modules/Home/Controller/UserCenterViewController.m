...
- (void)configNavigation {
    NSArray *titles = @[@"聊天", @" 关注", @" 打赏"];
    NSArray *images = @[@"聊天", @"+", @"打赏"];
    NSArray *withs  = @[@50, @60, @50];
...
- (void)getUserBaseInfo:(BOOL)pushChatVC {
...
- (void)rewardUser {
    ...
    rewardVC.payForReward = ^(NSNumber *amount, NSString *payType, NSString *orderNo) {
        ...
        [AlertViewTool showAlertViewWithTitle:nil Message:message cancelTitle:@"取消" sureTitle:@"零钱支付" sureBlock:^{
            [weakself payByWalletWithOrderNo:orderNo payMethod:payType];
        } cancelBlock:^{
...
- (void)chatToUser {
    if (_userBaseInfoModel) {
        if ([LoginData sharedLoginData].audit == 1 ||
            ...
            __weak typeof(self) weakself = self;
            [AuthorityTool chatPermissionDeniedFromViewController:weakself returnBlock:^{
                [weakself rewardUser];
            }];
        }
    } else {
        [self getUserBaseInfo:YES];
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section) {
        ...
        if (model.feedsType == 1) {
            ...
            cell.actionSheetItemClicked = ^(NSUInteger buttonIndex, NSString *trendsId, int goodsType, NSIndexPath *indexPath) {
                if (buttonIndex != 0) {
                    [weakself popToReportViewControllerWithType:ReportType_Trends aboutID:trendsId];
                }
            };
        ...
        cell.actionSheetItemClicked = ^(NSUInteger buttonIndex) {
            if (buttonIndex == 1) {
                ...
                // 举报操作
                [weakself popToReportViewControllerWithType:ReportType_Person aboutID:weakself.userBaseInfoModel.userId];
            }
        };
        ...
        cell.shareButtonClickedBlock = ^{
            ...
            shareVC.tapShareItem = ^(NSInteger index) {
                [ShareTool shareToPlatformType:index viewController:weakself];
            };
