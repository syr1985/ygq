...
NSString * const kNotification_DeleteTrendsSuccess = @"kNotification_DeleteTrendsSuccess";
...
#pragma mark -
#pragma mark - 加载数据
- (void)loadDetailInfoFromServer:(BOOL)updateFavoursList {
    __weak typeof(self) weakself = self;
    [NetworkTool getTrendsDetailWithTreendsID:_homeFocusModel.focusId success:^(id result) {
        weakself.homeFocusModel = [HomeFocusModel homeFocusModelWithDict:result[@"detailInfo"]];
        weakself.favoursListArray = result[@"feedZanList"];
        weakself.homeFocusModel.praise = [result[@"isMyZan"] boolValue];
        weakself.homeFocusModel.isBuy  = [result[@"isBuy"] boolValue];
        [weakself.tableView beginUpdates];
        if (updateFavoursList) {
...
- (void)loadNewData {
    ...
    [self loadDetailInfoFromServer:NO];
}
...
- (void)loadCommentDataFromServer {
    ...
    [NetworkTool getCommentListWithPageNO:@(_pageNo) pageSize:@(_pageSize) trendsId:_homeFocusModel.focusId success:^(id result) {
        ...
        weakself.tableView.mj_footer.hidden = muArray.count < self.pageSize;
...
- (void)popupKeyBoard {
    _currentCommentID = @"0";
    _currentUserId = _homeFocusModel.userId;
    _keyBoard.placeHolder = @"说点什么吧";
...
- (void)chatKeyBoardSendText:(NSString *)text {
    ...
    // 发布评论
    __weak typeof(self) weakself = self;
    [NetworkTool doCommentWithContent:text toUser:_currentUserId toComment:_currentCommentID toProduct:_homeFocusModel.focusId success:^{
        NSUInteger commentCount = [weakself.homeFocusModel.commentCount integerValue] ;
        NSNumber *newCommentCount = @(commentCount + 1);
        [weakself updateLocalData:newCommentCount doComment:YES commentID:nil];
    }];
}

- (void)updateLocalData:(NSNumber *)newCommentCount doComment:(BOOL)comment commentID:(NSString *)commentId {
    self.homeFocusModel.commentCount = newCommentCount;
    ...
    if (comment) {
        ...
        [self.commentListArray enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            DetailCommentModel *model = (DetailCommentModel *)obj;
            if ([model.commentId isEqualToString:commentId]) {
                *stop = YES;
                [weakself.commentListArray removeObjectAtIndex:idx];
                [weakself.tableView beginUpdates];
                [weakself.tableView reloadSections:[NSIndexSet indexSetWithIndex:1] withRowAnimation:UITableViewRowAnimationNone];
                [weakself.tableView endUpdates];
            }
...
- (void)popToReportViewControllerWithType:(NSUInteger)type aboutID:(NSString *)aboutID {
    ...
    reportVC.reportType = (ReportType)type;
...
- (void)handelCommentOperationWithType:(MenuType)type index:(NSUInteger)index comment:(DetailCommentModel *)model {
    switch (type) {
        ...
        case MenuType_LongPressSelfComment: {
            if (index == 1) {
                ...
                [NetworkTool deleteCommentWithFeedsID:_homeFocusModel.focusId commentID:model.commentId success:^{
                    ...
                    NSUInteger commentCount = [weakself.homeFocusModel.commentCount integerValue] ;
                    NSNumber *newCommentCount = @(commentCount - 1);
                    [weakself updateLocalData:newCommentCount doComment:NO commentID:model.commentId];
                }];
            }
        }
            break;
        case MenuType_LongPressSelfTrends: {
            ...
            break;
        case MenuType_LongPressSelfTrendsAndSelfComment: {
            if (index == 1) {
                // 复制
                _copiedComment = model.commentContent;
                UIPasteboard *pasteboard = [UIPasteboard generalPasteboard];
    ...
    if (indexPath.section == 0) {
        if (indexPath.row == 0) {
            if (_homeFocusModel.feedsType == 1) {
                ...
                cell.favourResultBlock = ^(BOOL cancel) {
                    [weakself loadDetailInfoFromServer:YES];
                };
                ...
                cell.isTrendsDetail = YES;
                if (_homeFocusModel.videoUrl && _homeFocusModel.videoUrl.length) {
                    cell.homeFocusModel = _homeFocusModel;
                }
                return cell;
        ...
        if ([model.aboutCommentId isEqualToString:@"0"]) {
            ...
            cell.tapCommentBlock = ^(DetailCommentModel *model) {
                weakself.currentUserId = model.fromUserId;
                weakself.currentCommentID = model.commentId;
                weakself.keyBoard.placeHolder = [NSString stringWithFormat:@"回复：%@",model.fromUserNickName];
                [weakself.keyBoard beginComment];
            };
            cell.longPressCommentBlock = ^(DetailCommentModel *model) {
                MenuType type = MenuType_LongPressOtherComment;
                if ([weakself.homeFocusModel.userId isEqualToString:[LoginData sharedLoginData].userId]) {
                    type = MenuType_LongPressSelfTrends;
                    if ([model.fromUserId isEqualToString:[LoginData sharedLoginData].userId]) {
                        type = MenuType_LongPressSelfTrendsAndSelfComment;
                    }
            ...
            cell.tapHeadImageViewBlock = ^(NSString *userId) {
                [weakself popToUserCenterViewController:userId];
            };
