...
- (void)setupTextSnapView {
    ...
    [self.snapImageView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.equalTo(weakself.snapBackView.mas_right).with.offset(-12);
    ...
    [self.snapLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(weakself.snapBackView.mas_left).with.offset(12);
...
- (void)addCountDownButton:(BOOL)startCountDown {
    // 添加右上角的锁
    __weak typeof(self) weakself = self;
    ...
    if (startCountDown) {
        ...
        [self.countDownButton setBackgroundColor:NavTabBarColor];
