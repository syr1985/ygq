...
+ (void)initialize {
    ...
    cell.leftBubbleMargin = UIEdgeInsetsMake(9, 24, 9, 12);//(2, 12, 2, 2);
    cell.rightBubbleMargin = UIEdgeInsetsMake(9, 12, 9, 24);//(2, 2, 2, 12);
    cell.bubbleMargin = UIEdgeInsetsMake(8, 0, 8, 0);
    
    cell.messageTextFont = [UIFont systemFontOfSize:16];
    cell.messageTextColor = FontColor;//[UIColor blackColor];
    
    cell.messageLocationFont = [UIFont systemFontOfSize:10];
    cell.messageLocationColor = FontColor;//[UIColor whiteColor];
    
    cell.messageVoiceDurationColor = FontColor;//[UIColor grayColor];
    cell.messageVoiceDurationFont = [UIFont systemFontOfSize:16];
    ...
    [cell setSendBubbleBackgroundImage:[[UIImage imageNamed:@"气泡-我"] stretchableImageWithLeftCapWidth:25 topCapHeight:25]];
    [cell setRecvBubbleBackgroundImage:[[UIImage imageNamed:@"气泡-他人"] stretchableImageWithLeftCapWidth:25 topCapHeight:25]];
}
...
- (void)_setupSubviewsWithType:(EMMessageBodyType)messageType
    ...
    [_statusButton setImage:[UIImage imageNamed:@"消息-警示"] forState:UIControlStateNormal];
    ...
    _hasRead = [UIButton buttonWithType:UIButtonTypeCustom];
    _hasRead.translatesAutoresizingMaskIntoConstraints = NO;
    _hasRead.titleLabel.font = [UIFont systemFontOfSize:10];
    [_hasRead setBackgroundImage:[UIImage imageNamed:@"已读bg"] forState:UIControlStateNormal];
    [_hasRead setTitle:NSEaseLocalizedString(@"hasRead", @"Read") forState:UIControlStateNormal];
    _hasRead.hidden = YES;
    [_hasRead sizeToFit];
    ...
    if ([self respondsToSelector:@selector(isCustomBubbleView:)] && [self isCustomBubbleView:model]) {
        ...
        switch (messageType) {
            ...
            case EMMessageBodyTypeLocation:
            {
                ...
                UIImage *phImage = [[UIImage imageNamed:@"EaseUIResource.bundle/chat_location_preview"] stretchableImageWithLeftCapWidth:10 topCapHeight:10];
                EMMessage *message = model.message;
                NSDictionary *ext = message.ext;
                NSString *imageUrl = ext[@"MapViewImage"];
                [_bubbleView.locationImageView sd_setImageWithURL:[NSURL URLWithString:imageUrl]
                                                 placeholderImage:phImage];
                NSString *detailAddress = ext[@"DetailAddress"];
                if (detailAddress) {
                    _bubbleView.detailLocationLabel.text = detailAddress;
                }
...
- (void)_setupConstraints {
    ...
    self.bubbleMaxWidthConstraint = [NSLayoutConstraint constraintWithItem:self.bubbleView attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationLessThanOrEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1.0 constant:200];
    ...
    [self addConstraint: [NSLayoutConstraint constraintWithItem:_hasRead attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:0 constant:14]];
}
...
- (void)_updateHasReadWidthConstraint {
    if (_hasRead) {
        ...
        self.hasReadWidthConstraint = [NSLayoutConstraint constraintWithItem:_hasRead attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:0 constant:29];
        ...
        switch (model.bodyType) {
            ...
            case EMMessageBodyTypeImage:
            {
                ...
                if (!image) {
                    image = _model.image;
                    if (!image) {
                        [_bubbleView.imageView sd_setImageWithURL:[NSURL URLWithString:_model.fileURLPath]
                                                 placeholderImage:[UIImage imageNamed:_model.failImageName]
                                                        completed:^(UIImage *image, NSError *error, EMSDImageCacheType cacheType, NSURL *imageURL) {
                                                            ...
                                                            imageBody.downloadStatus = EMDownloadStatusSuccessed;
                                                        }];
                    } else {
            ...
            case EMMessageBodyTypeLocation:
            {
                if ([_model.address containsString:@"LOCATIONNAME="]) {
                    NSArray *contentArray = [_model.address componentsSeparatedByString:@"LOCATIONNAME="];
                    _bubbleView.locationLabel.text = contentArray[1];
                    _bubbleView.detailLocationLabel.text = contentArray[0];
                } else {
            ...
            case EMMessageBodyTypeVoice:
            {
                if (_bubbleView.voiceImageView) {
                    if (self.model.isSender) {
                        self.bubbleView.voiceImageView.image = [UIImage imageNamed:@"语音气泡-自己"];
                    } else {
                        self.bubbleView.voiceImageView.image = [UIImage imageNamed:@"语音气泡-他人"];
                    }
                }
                if (!self.model.isSender) {
                    _bubbleView.isReadView.hidden = self.model.isMediaPlayed;
                }
            ...
            case EMMessageBodyTypeVideo:
            {
                ...
                if (!image) {
                    image = _model.image;
                    if (!image) {
                        [_bubbleView.videoImageView sd_setImageWithURL:[NSURL URLWithString:_model.fileURLPath]
                                                      placeholderImage:[UIImage imageNamed:_model.failImageName]];
                    } else {
    ...
    if ([self respondsToSelector:@selector(isCustomBubbleView:)] && [self isCustomBubbleView:_model]) {
        ...
        if (_bubbleView) {
            switch (_messageType) {
                ...
                case EMMessageBodyTypeImage:
                {
                    UIEdgeInsets bubbleMargin = self.model.isSender ? UIEdgeInsetsMake(0, 0, 0, 0) : UIEdgeInsetsMake(0, 0, 0, 0);
                    [_bubbleView updateImageMargin:bubbleMargin];
                }
                    break;
                case EMMessageBodyTypeLocation:
                {
                    UIEdgeInsets bubbleMargin = self.model.isSender ? UIEdgeInsetsMake(0, 0, 0, 10) : UIEdgeInsetsMake(0, 10, 0, 0);
                    [_bubbleView updateLocationMargin:bubbleMargin];
                }
                    break;
                case EMMessageBodyTypeVoice:
                {
...
+ (CGFloat)cellHeightWithModel:(id<IMessageModel>)model {
    ...
    CGFloat height = EaseMessageCellPadding + 18;//cell.bubbleMargin.top + cell.bubbleMargin.bottom;
...
//            height += [NSString heightWithString:model.text maxSize:CGSizeMake(bubbleMaxWidth, CGFLOAT_MAX) strFont:cell.messageTextFont];
