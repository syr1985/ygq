...
//        NSLog(@"INVALID: %@", productRequestNotification.invalidProductIds);
    ...
    } else if (result == APProductRequestFailure) {
        [AlertViewTool showAlertViewWithTitle:@"内购提示"
                                      Message:productRequestNotification.errorMessage
                                  buttonTitle:@"确定"
                                    sureBlock:nil];
    }
}
...
- (void)verifyReceipts {
    ...
    if (receiptData) {
        ...
        [NetworkTool verifyReceiptWithReceipt:encodeStr orderNo:_orderNo success:^{
            ...
            if (weakself.payFailureBlock) {
                weakself.payFailureBlock();
            }
        }];
    }
}
...
- (void)saveReceipt {
    ...
    NSString *savePath = [NSString stringWithFormat:@"%@%@_%@.plist", AppStoreInfoLocalFilePath, fileName, dateString];
...
//    NSData *receiptData = [[NSData alloc] initWithBase64EncodedString:receipt
//                                                              options:NSDataBase64DecodingIgnoreUnknownCharacters];
    //[self localVerifyReceipt:receiptData];
    __weak typeof(self) weakself = self;
    [NetworkTool verifyReceiptWithReceipt:receipt orderNo:orderNo success:^{
        [weakself removeIapFailedPath:plistPath];
    } failure:nil];
}
...
- (void)removeIapFailedPath:(NSString *)plistPath {
    ...
    if ([fileManager fileExistsAtPath:plistPath]) {
        BOOL removed = [fileManager removeItemAtPath:plistPath error:nil];
        NSLog(@"%@ remove = %d",plistPath, removed);
    }
}
...
- (void)localVerifyReceipt:(NSData *)receiptData {
    NSString *encodeStr = [receiptData base64EncodedStringWithOptions:NSDataBase64EncodingEndLineWithLineFeed];
