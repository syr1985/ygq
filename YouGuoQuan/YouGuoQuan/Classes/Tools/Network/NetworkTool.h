 ...
 *  获取我售出的订单
 *
 *  @param dealType  订单类型 1、已处理，2、未处理， @""、全部
 *  @param pageNo    页号
 *  @param pageSize  每页数
 *  @param success   成功block
 *  @param failure   失败block
 */
+ (void)getMySoldOrderListWithType:(NSString *)dealType pageNo:(NSNumber *)pageNo pageSize:(NSNumber *)pageSize success:(void (^)(id result))success failure:(void(^)())failure;
/**
 *  申请退款
 *
 *  @param orderNo 订单号
