...
- (void)setCoordinate:(CLLocationCoordinate2D)coordinate {
    ...
    if (_city && [LoginData sharedLoginData].userId) {
        ...
        [NetworkTool updateUserLocationInfoWithCity:_city
                                           latitude:lat
                                          longitude:lon
                                            success:nil
                                            failure:nil];
...
- (void)locationManager:(CLLocationManager *)manager didFailWithError:(NSError *)error {
    ...
    if (error.code == kCLErrorDenied) {
        ...
        [AlertViewTool showAlertViewWithTitle:@"访问位置信息被拒绝" Message:@"用户已设置不允许访问位置信息，是否去重新设置？" cancelTitle:@"取消" sureTitle:@"设置" sureBlock:^{
...
- (void)locationManager:(CLLocationManager *)manager didChangeAuthorizationStatus:(CLAuthorizationStatus)status {
    switch (status) {
        ...
        {
            // 提示用户出错原因
            [SVProgressHUD showInfoWithStatus:@"请在设置中开启定位功能！"];
        }
            break;
