...
//    NSString *pattern = @"^[A-Za-z0-9]{6,20}+$";
    NSString *pattern = @"^[\\u4e00-\\u9fa5_a-zA-Z0-9-]{1,16}$";//@"^([\u4e00-\u9fa5]+|([a-zA-Z]+\s?)+)$";
    NSPredicate *pred = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", pattern];
...
// 1~100
+ (BOOL) checkAge:(NSString *)age {
    NSString *bankNum = @"^(?:[1-9]?\\d|100)$";
    NSPredicate *pred = [NSPredicate predicateWithFormat:@"SELF MATCHES %@",bankNum];
    BOOL isMatch = [pred evaluateWithObject:age];
    return isMatch;
}

// 1~300cm
+ (BOOL) checkHeight:(NSString *)height {
    NSString *bankNum = @"\\b[1-9]\\d{0，1}\\b|\\b[1-2]\\d\\d\\b";
    NSPredicate *pred = [NSPredicate predicateWithFormat:@"SELF MATCHES %@",bankNum];
    BOOL isMatch = [pred evaluateWithObject:height];
    return isMatch;
}

// 1~400kg
+ (BOOL) checkWeight:(NSString *)weight {
    NSString *bankNum = @"\\b[1-9]\\d{0，1}\\b|\\b[1-3]\\d\\d\\b";
    NSPredicate *pred = [NSPredicate predicateWithFormat:@"SELF MATCHES %@",bankNum];
    BOOL isMatch = [pred evaluateWithObject:weight];
    return isMatch;
}
