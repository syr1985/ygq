...
- (NSString *)description {
    ...
    if (_month != 0) {
        return [NSString stringWithFormat:@"%zd月", _month];
    }
    
    if (_week != 0) {
        return [NSString stringWithFormat:@"%zd周", _week];
    }
    
    if (_day != 0) {
        return  [NSString stringWithFormat:@"%zd天", _day];
    }
    
    if (_hour != 0) {
        return  [NSString stringWithFormat:@"%zd小时", _hour];
    }
    
    if (_minute != 0) {
        return [NSString stringWithFormat:@"%zd分钟", _minute];
    }
    
    return @"刚刚";//[NSString stringWithFormat:@"%zd秒", _second];
}
...
- (LXDateItem *)lx_timeIntervalSinceDate:(NSDate *)anotherDate {
    ...
    // 相差多少周
    long secondsPerMonth = 30 * 24 * 3600;
    item.month = intInterval / secondsPerMonth;
    
    // 相差多少周
    long secondsPerWeek = 7 * 24 * 3600;
    item.week = intInterval / secondsPerWeek;
