...
- (void)initialize {
    ...
    self.reusableCells = [NSMutableArray new];
        ...
        case NewPagedFlowViewOrientationHorizontal: {
            ...
            for (NSUInteger i = self.visibleRange.location; i < self.visibleRange.location + _visibleRange.length; i++) {
                ...
                if (delta < _pageSize.width) {
                    cell.coverView.alpha = 0;//(delta / _pageSize.width) * (1 - _minimumPageAlpha);
                    ...
                    cell.coverView.alpha = 0;//_minimumPageAlpha;
...
- (void)setPagesAtContentOffset:(CGPoint)offset {
    ...
    switch (self.orientation) {
        case NewPagedFlowViewOrientationHorizontal:{
            ...
            for (NSInteger i = startIndex; i < cellCount; i++) {
                //如果都不超过则取最后一个
                if ((_pageSize.width * (i + 1) < endPoint.x && _pageSize.width * (i + 2) >= endPoint.x) || (i + 2) == cellCount) {
            ...
            endIndex = MIN(endIndex + 1, cellCount - 1);
            ...
            for (NSInteger i = endIndex + 1; i < cellCount; i++) {
                [self removeCellAtIndex:i];
            }
            break;
        }
        case NewPagedFlowViewOrientationVertical: {
            ...
            for (NSInteger i = startIndex; i < cellCount; i++) {
                //如果都不超过则取最后一个
                if ((_pageSize.height * (i + 1) < endPoint.y && _pageSize.height * (i + 2) >= endPoint.y) || (i + 2) == cellCount) {
                    endIndex = i + 1;//i+2 是以个数，所以其index需要减去1
                    break;
                }
            }
...
- (void)reloadData {
    ...
    for (UIView *view in self.scrollView.subviews) {
        if ([NSStringFromClass(view.class) isEqualToString:@"IndexBannerSubiew"]) {
    ...
    if (_needsReload) {
        ...
        switch (self.orientation) {
            case NewPagedFlowViewOrientationHorizontal://横向
                ...
                CGFloat deltY = (self.bounds.size.height - _pageSize.height) * 0.5;
                CGPoint theCenter = CGPointMake(CGRectGetMidX(self.bounds), CGRectGetMidY(self.bounds) - deltY);
