...
- (void)setDetailEvaluateModel:(ProductEvaluateModel *)detailEvaluateModel {
    ...
    NSString *headImageUrlStr = [NSString compressImageUrlWithUrlString:detailEvaluateModel.headImg
    ...
    self.levelImageView.hidden = detailEvaluateModel.star == 0;
    if (detailEvaluateModel.star != 0) {
...
- (void)setInfoDict:(NSDictionary *)infoDict {
    ...
    _totalCommentCountLabel.text = [NSString stringWithFormat:@"共%@条评论",cmtCount];
    //[NSString stringWithFormat:@"共%@条评论（好评率%@%%）",cmtCount,infoDict[@"praiseRate"]];
}
