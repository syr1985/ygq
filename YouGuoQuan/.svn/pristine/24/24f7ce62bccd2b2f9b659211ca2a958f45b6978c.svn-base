...
- (IBAction)payNowForCrowdfunding {
    ...
    __weak typeof(self) weakself = self;
    [AlertViewTool showAlertViewWithTitle:@"" Message:@"直至活动结束，您预留的手机与邮箱不可变更，请您确认你预留的是正确号码" cancelTitle:@"取消" sureTitle:@"确定" sureBlock:^{
        [NetworkTool createCrowdfundingOrderWithCfId:_discoveryCrowdfundingModel.crowdfundingId payMethod:payType money:_inputPrice phone:_inputTelNum email:_inputEmail orderNo:_orderNo success:^(id result, id payOrderNo) {
            if (weakself.currentPayIndex == 2) {
                [NetworkTool payforCrowdfundingWithOrderNo:result success:^{
                    [weakself payResultForSuccess];
