...
- (void)setup{
    ...
    contentLabel.textColor = FontColor;//JCAlertViewContentColor;
    contentLabel.font = [UIFont systemFontOfSize:13];//JCAlertViewContentFont;
    ...
    if (count == 1) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            CGFloat btnY = self.frame.size.height - JCButtonHeight;// - JCMargin;
            CGRect btnFrame = CGRectMake(btnWidth * i, btnY, btnWidth, JCButtonHeight);
            UIButton *btn = [[UIButton alloc] initWithFrame:btnFrame];
            //CGRectMake(JCMargin + (JCMargin + btnWidth) * i, self.frame.size.height - JCButtonHeight - JCMargin, btnWidth, JCButtonHeight)];
            NSDictionary *btnDict = self.buttons[i];
            NSString *title = [btnDict.allValues firstObject];
            ...
            btn.layer.borderColor = [UIColor colorWithWhite:0.9 alpha:0.7].CGColor;
        ...
        for (int i = 0; i < count; i++) {
            CGRect btnFrame = CGRectMake(JCMargin, JCAlertViewTitleLabelHeight + contentHeight + JCMargin + (JCMargin + JCButtonHeight) * i, JCAlertViewWidth - JCMargin * 2, JCButtonHeight);
            UIButton *btn = [[UIButton alloc] initWithFrame:btnFrame];
            NSDictionary *btnDict = self.buttons[i];
            [btn setTitle:[btnDict.allValues firstObject] forState:UIControlStateNormal];
            [self setButton:btn BackgroundWithButonType:[[btnDict.allKeys firstObject] integerValue]];
