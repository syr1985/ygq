...
- (void)setbarSubViewsFrame
{
    ...
    self.textView.frame = CGRectMake(CGRectGetWidth(self.switchBarBtn.frame) + CGRectGetWidth(self.voiceBtn.frame) + 8, TextViewVerticalOffset, self.frame.size.width-CGRectGetWidth(self.switchBarBtn.frame)-CGRectGetWidth(self.voiceBtn.frame)-CGRectGetWidth(self.faceBtn.frame)-CGRectGetWidth(self.moreBtn.frame) - 8, self.textView.frame.size.height);
...
- (void)loadBarItems:(NSArray<ChatToolBarItem *> *)barItems
{
    ...
    {
        [self setBtn:(ButKind)barItem.itemKind normalStateImageStr:barItem.normalStr selectStateImageStr:barItem.selectStr highLightStateImageStr:barItem.highLStr];
    }
}
