...
@property (nonatomic, assign) BOOL bShowCloseButton;
@end
...
- (void)setup {
    ...
    _realTimeBlur.blurStyle = XHBlurStyleWhiteTranslucent;
    ...
    _realTimeBlur.didDismissBlurViewCompleted = ^(BOOL finished) {
        ...
        if (finished) {
            if (strongself.didSelectedItemCompletion) {
                strongself.didSelectedItemCompletion(strongself.selectedItem);
...
- (void)showMenuAtView:(UIView *)containerView showCloseButton:(BOOL)close {
    self.bShowCloseButton = close;
...
- (void)showButtons {
    ...
    for (int index = 0; index < items.count; index ++) {
        ...
        // 判断如果不能被整除，则加一行
        NSUInteger exItemCount = items.count % perRowItemCount != 0 ? 1 : 0;
        NSInteger preColumItemCount = items.count / perRowItemCount + exItemCount;
        CGRect toRect = [self getFrameWithItemCount:items.count
                                    perRowItemCount:perRowItemCount
                                  perColumItemCount:preColumItemCount
        ...
        if (index == items.count - 1 && self.bShowCloseButton) {
            CGFloat itemY = toRect.origin.y + MenuButtonHeight;
            CGFloat itemX = self.bounds.size.width * 0.5 - 12;
            CGFloat maxY  = [UIScreen mainScreen].bounds.size.height;
            if (itemY >= maxY) {
                itemY = maxY - 48;
            }
            toRect = CGRectMake(itemX, itemY, 24, 24);
        }
        ...
        if (!menuButton) {
            ...
            //                closeButton.frame = CGRectMake(0, 0, fromRect.size.width, fromRect.size.height);
            //                [closeButton setImage:[UIImage imageNamed:@"发布-关闭"] forState:UIControlStateNormal];
            //                [closeButton setImage:[UIImage imageNamed:@"发布关闭-点击"] forState:UIControlStateHighlighted];
            //                //[closeButton addTarget:self action:@selector(dismissMenu) forControlEvents:UIControlEventTouchUpInside];
            //                [menuButton addSubview:closeButton];
            ...
            menuButton.didSelctedItemCompleted = ^(MenuItem *menuItem) {
                __strong typeof(self) strongself = weakSelf;
                strongself.selectedItem = menuItem;
                [strongself dismissMenu];
            };
...
- (CGRect)getFrameWithItemCount:(NSInteger)itemCount
    ...
    CGFloat insetY = CGRectGetHeight(self.bounds) - (itemHeight + paddingY) * rowCount - 48;
