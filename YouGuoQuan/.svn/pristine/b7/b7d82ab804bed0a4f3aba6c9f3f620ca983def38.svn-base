...
// Skip the WAVE header to PCM audio data 从WAVE文件中跳过WAVE文件头，直接到PCM音频数据
static void SkipToPCMAudioData(FILE* fpwave)
{
    ...
    // 1. Read the RIFF header 读RIFF头
    fread(&riff, 1, sizeof(EM_RIFFHEADER), fpwave);
    
    // 2.  Read the FMT chunk - if fmt.nFmtSize>16, read the remaining MATX 读FMT块 - 如果 fmt.nFmtSize>16 说明需要还有一个附属大小没有读
    ...
    // 3.Switch to the data block 转到data块 - 有些还有fact块等
    while(!bDataBlock)
    {
        fread(&chunk, 1, sizeof(EM_XCHUNKHEADER), fpwave);
        if ( memcmp(chunk.chChunkID, "data", 4) == 0 )
        {
            bDataBlock = 1;
            break;
        }
