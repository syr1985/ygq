...
- (void)confirmViewController {
    ...
    [_getAuthCodeButton countDownChanging:^NSString *(JKCountDownButton *countDownButton,NSUInteger second) {
        NSString *title = [NSString stringWithFormat:@"重新获取验证码(%zd)",second];
        return title;
    }];
    [_getAuthCodeButton countDownFinished:^NSString *(JKCountDownButton *countDownButton, NSUInteger second) {
    ...
    [_getAuthCodeButton startCountDownWithSecond:60];
