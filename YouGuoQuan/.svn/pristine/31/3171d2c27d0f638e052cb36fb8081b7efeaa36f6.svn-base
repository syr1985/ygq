...
- (void)initWMPlayer {
    ...
    [self.playOrPauseBtn setImage:[UIImage imageNamed:@"暂停按钮"] forState:UIControlStateNormal];//WMPlayerImage(@"pause")
    [self.playOrPauseBtn setImage:[UIImage imageNamed:@"开始按钮"] forState:UIControlStateSelected];//WMPlayerImage(@"play")
    [self.bottomView addSubview:self.playOrPauseBtn];
    [self.playOrPauseBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.equalTo(self.bottomView.mas_centerY).with.offset(-35);
    ...
    //进度条的拖拽事件
    [self.progressSlider addTarget:self action:@selector(stratDragSlide:)  forControlEvents:UIControlEventTouchDragEnter];
    //进度条的点击事件
    [self.progressSlider addTarget:self action:@selector(updateProgress:) forControlEvents:UIControlEventTouchDragInside];
    //
    [self.progressSlider addTarget:self action:@selector(exitDragSlide:)  forControlEvents:UIControlEventTouchDragExit];
    //给进度条添加单击手势
    ...
    [self.progressSlider mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.leftTimeLabel.mas_right).with.offset(8);
        make.right.equalTo(self.rightTimeLabel.mas_left).with.offset(-8);
        make.centerY.equalTo(self.leftTimeLabel.mas_centerY);
    }];
    ...
    //    [self.closeBtn addTarget:self action:@selector(closeTheVideo:) forControlEvents:UIControlEventTouchUpInside];
    //    [self.topView addSubview:self.closeBtn];
    //    [self.closeBtn mas_makeConstraints:^(MASConstraintMaker *make) {
    //        make.left.equalTo(self.topView.mas_left).with.offset(8);
    //        make.top.equalTo(self.topView.mas_top).with.offset(8);
    //    }];
    ...
    [self.titleLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.right.equalTo(self.topView.mas_right).with.offset(-8);
    }];
...
- (void)appDidEnterBackground:(NSNotification*)note {
    if (!self.playOrPauseBtn.isSelected) {//如果是播放中，则继续播放
...
- (void)closeTheVideo:(UIButton *)sender {
    ...
    if (self.isFullscreen) {
        [self.titleLabel mas_updateConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(self.closeBtn).with.offset(8);
        }];
    }
}
...
- (void)PlayOrPause:(UIButton *)sender {
    if (self.state == WMPlayerStateStopped || self.state == WMPlayerStateFailed) {
        ...
        [self pause];
    } else if(self.state == WMPlayerStateFinished || self.state == WMPlayerStatePause) {
        self.state = WMPlayerStatePlaying;
        [self.player play];
        self.playOrPauseBtn.selected = NO;
    }
...
- (void)play {
    if (!self.isInitPlayer) {
        ...
        if (self.state == WMPlayerStateStopped || self.state == WMPlayerStatePause || self.state == WMPlayerStateFinished) {
            self.state = WMPlayerStatePlaying;
            [self.player play];
            self.playOrPauseBtn.selected = NO;
        } 
...
- (void)setCloseBtnStyle:(CloseBtnStyle)closeBtnStyle {
    ...
    //        [_closeBtn setImage:[UIImage imageNamed:@"返回-白"] forState:UIControlStateNormal];//WMPlayerImage(@"play_back.png")
    //        [_closeBtn setImage:[UIImage imageNamed:@"返回-白"] forState:UIControlStateSelected];
    //    } else {
    //        [_closeBtn setImage:nil forState:UIControlStateNormal];
    //        [_closeBtn setImage:nil forState:UIControlStateSelected];
    //    }
}
/**
 *  设置播放的状态
...
- (void)exitDragSlide:(UISlider *)slider {
...
- (void)autoDismissBottomView:(NSTimer *)timer{
    if (self.state == WMPlayerStatePlaying) {
        if (self.bottomView.alpha == 1.0 && !self.isDragingSlider) {
...
- (void)initTimer {
    ...
    self.playbackTimeObserver =  [self.player addPeriodicTimeObserverForInterval:CMTimeMakeWithSeconds(1.0, NSEC_PER_SEC)
...
- (void)syncScrubber {
    ...
    if (!self.isDragingSlider) {//拖拽slider中，不更新slider的值
