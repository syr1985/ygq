...
- (void)configureSendLayoutConstraints {
    ...
    [self.bubbleView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.equalTo(weakself.avatarView.mas_left).with.offset(-2);
...
- (void)configureRecvLayoutConstraints {
    ...
    //bubble view
    [self.bubbleView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(weakself.avatarView.mas_right).with.offset(2);
        make.top.equalTo(weakself.avatarView.mas_top);
    }];
}
...
- (void)setInfoDict:(NSDictionary *)infoDict {
    ...
    NSString *headImageUrlStr = [NSString compressImageUrlWithUrlString:infoDict[@"headImg"]
                                                                  width:self.avatarView.bounds.size.width
                                                                 height:self.avatarView.bounds.size.height];
    [self.avatarView sd_setImageWithURL:[NSURL URLWithString:headImageUrlStr]
...
- (void)setModel:(id<IMessageModel>)model {
    ...
    if (model.isSender) {
        ...
        self.vipView.hidden = [LoginData sharedLoginData].audit != 1 && [LoginData sharedLoginData].audit != 3;
        self.vipView.image =  [LoginData sharedLoginData].audit == 3 ? [UIImage imageNamed:@"列表头像团体认证"] : [UIImage imageNamed:@"头像加V"];
        self.recommandView.hidden = [LoginData sharedLoginData].star != 6;
    ...
    if (model.avatarURLPath) {
        NSString *headImageUrlStr = [NSString compressImageUrlWithUrlString:model.avatarURLPath
                                                                      width:self.avatarView.bounds.size.width
                                                                     height:self.avatarView.bounds.size.height];
        [self.avatarView sd_setImageWithURL:[NSURL URLWithString:headImageUrlStr]
                           placeholderImage:[UIImage imageNamed:@"my_head_default"]];
