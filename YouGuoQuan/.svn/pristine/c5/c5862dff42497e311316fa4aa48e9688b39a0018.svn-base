...
- (void)setModel:(id<IConversationModel>)model
{
    ...
    if (self.showAvatar) {
        if ([_model.avatarURLPath length] > 0){
            NSString *headImageUrlStr = [NSString compressImageUrlWithUrlString:_model.avatarURLPath
                                                                          width:self.avatarView.bounds.size.width
                                                                         height:self.avatarView.bounds.size.height];
            [self.avatarView.imageView sd_setImageWithURL:[NSURL URLWithString:headImageUrlStr]
