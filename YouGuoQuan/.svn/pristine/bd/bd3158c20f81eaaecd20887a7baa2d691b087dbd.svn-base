...
- (void)prepareLayout {
    ...
    //计算每个section的停留时与顶部的距离通,过代理方法（collectionView:layout:headerToTopInSection:）获得
    ...
    //计算所在section与上一个section的间距和所在section与下一个section的间距.通过代理方法（collectionView:layout:topInSection:和collectionView:layout:bottomInSection:）获得
...
- (NSNumber*) calculateHeightForSection: (NSInteger)section {
    ...
    for (NSInteger item = 0; item < itemCount; item++) {
        ...
        for (NSInteger column = 1; column < [self.columnsCountArray[section] integerValue]; column++) {
