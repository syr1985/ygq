...
- (void)resizeSubviews {
    ...
    CGFloat scrollViewContentH = MAX(_imageContainerView.tz_height, self.tz_height);
    CGFloat deltaH = 0;
    if (_cropRectHeight > 0) {
        deltaH = _imageView.tz_height - _cropRectHeight;
        scrollViewContentH += deltaH;
    }
    
    _scrollView.contentSize = CGSizeMake(self.scrollView.tz_width, scrollViewContentH);
    [_scrollView scrollRectToVisible:CGRectMake(0, deltaH * 0.5, self.bounds.size.width, self.bounds.size.height) animated:NO];
    _scrollView.alwaysBounceVertical = _imageContainerView.tz_height <= self.tz_height ? NO : YES;
    _imageView.frame =  _imageContainerView.bounds;
    _imageContainerView.tz_centerY = self.scrollView.contentSize.height / 2;
}
